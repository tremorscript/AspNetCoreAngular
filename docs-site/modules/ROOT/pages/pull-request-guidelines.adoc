...
** `git publish` - Pushes the local branch updates to the current branch at origin. Rejects if the local branch is not up to date with the latest from origin.
** `git synced` - Updates your local branch from `origin/main` i.e. Github. It performs a rebase, replaying your commits on top of the tip of main or master.
** `git update1` - Updates your local branch with the latest from the same branch at origin. This fetches `origin/your-branch` and performs a rebase.
** `git squash` - Shrinks the many commits in a branch to one commit. Useful just before merging.
